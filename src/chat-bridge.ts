...
export interface OssConversationMessage {
  ...
  result?: { ok?: boolean; summary?: string; [key: string]: unknown }
  [key: string]: unknown
}

/** maplay chat request shape (mirror of maplay's OssChatRequest). */
export interface OssChatRequest {
  scene?: { mapName?: string; [key: string]: unknown }
  apiConfig?: { systemPrompt?: string; [key: string]: unknown }
...
export function maplayToolSchemas(): ToolSchema[] {
  return MAPLAY_TOOL_SPECS.map((spec) => ({
    name: spec.name,
    description: spec.description,
    parameters: parameterSchemaSpecToJsonSchema(spec.parameters) as unknown as Record<string, unknown>,
  }))
}
 ...
 */
export async function handleChatBridge(
  ctx: Context,
  llm: LlmRuntime,
  ...
  const toolCalls: OssChatResponse['toolCalls'] = blocks
