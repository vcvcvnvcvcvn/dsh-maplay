...
export function apply(ctx: Context, config: Config): void {
  ...
  const executor = ctx.get('maplay') as ToolExecutor | undefined
  if (executor === undefined) {
    throw new Error(
      'dsh-maplay-tools: the `maplay` executor service is unavailable. Mount the dsh-maplay host plugin (its index.js) before this preset half.',
    )
  }
