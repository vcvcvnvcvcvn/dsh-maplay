...
const string = (description?: string) => ({ type: 'string' as const, ...(description !== undefined ? { description } : {}) })
const req = <const T extends { type: string; description?: string }>(spec: T, description?: string): T & { required: true; description: string } =>
  ({ ...spec, ...(description !== undefined ? { description } : {}), required: true }) as T & { required: true; description: string }

export const MAPLAY_TOOL_SPECS: MaplayToolSpec[] = [
  {
    name: 'get_board_info',
